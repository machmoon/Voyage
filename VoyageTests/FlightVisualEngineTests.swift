...
final class WorldSceneryProviderPolicyTests: XCTestCase {
    func testOnlySelectedMapKitProviderMountsTheApple3DEngine() {
        ...
        XCTAssertFalse(WorldSceneryLayerPolicy.needsMapKit(under: .googleMaps3D))
    ...
    func testGoogleKeyValidationRejectsPlaceholdersAndMalformedCredentials() {
        XCTAssertFalse(WorldSceneryConfiguration.isUsableAPIKey(""))
        XCTAssertFalse(WorldSceneryConfiguration.isUsableAPIKey("$(GOOGLE_MAPS_API_KEY)"))
        XCTAssertFalse(WorldSceneryConfiguration.isUsableAPIKey("YOUR_API_KEY"))
        XCTAssertFalse(WorldSceneryConfiguration.isUsableAPIKey("not-a-google-key"))
        XCTAssertTrue(
            WorldSceneryConfiguration.isUsableAPIKey(
                "AIzaSyDeterministicTestCredential123456789"
            )
        )
    }

    func testGoogleIsPrimaryOnlyWhenEnabledOnlineKeyedAndThermallySafe() {
        XCTAssertEqual(
            WorldSceneryProviderPolicy.provider(
                ...
                hasGoogleMaps3DKey: true,
                googleMaps3DAllowed: true
            ),
            .googleMaps3D
        )
    }

    func testMissingKeyAndSeriousThermalPressureUseMapKit() {
        for (hasKey, thermalState) in [(false, ProcessInfo.ThermalState.nominal),
                                       (true, ProcessInfo.ThermalState.serious)] {
            XCTAssertEqual(
                WorldSceneryProviderPolicy.provider(
                    realWorldTwinEnabled: true,
                    isVisible: true,
