...
**Routes are real-world data.** `Voyage/Models/RouteCatalog.swift` hardcodes every directed airport pair: actual block times (directional — eastbound rides the jet stream and is shorter), flight numbers, nonstop vs. the popular connection (`via`), and typical daily departure times that feed the departure-board `ScheduleSheet`. `RoutePlanner.itinerary(from:to:flightNumberOverride:)` resolves everything through the catalog; the override stamps a booked departure's flight number onto the first leg. Seat labels are letter-first ("C10").

**The airline is fictional; everything else is real.** Airports, city names, coordinates, block times, and departure banks are facts and must stay accurate (see PLAN.md). The *carrier* is not: every route is flown by `Airline` in `RouteCatalog.swift` — trade name "Voyage Air", flight-number prefix `VOY`. It is the single source of truth for the boarding pass, the departure board, the Live Activity, the boarding push notification, the spoken PA, and Settings. **Never reintroduce a real airline's name or IATA code.** The app shipped with real carriers (United/UA, Delta/DL, American/AA, JetBlue/B6, Air Canada/AC, WestJet/WS) on the mock boarding pass; that is an App Review 5.2.5 (Intellectual Property) exposure, made worse by mixing them with the app's own "Voyage Air" branding. `VOY` is three letters on purpose — IATA airline designators are two characters, so it cannot collide with a live one.
...
**Support layer (`Voyage/Support/`):** `SettingsStore` (singleton; unit tests mute `ambienceEnabled`/`announcementsEnabled` in `setUp` to keep tests quiet), `FlightScheduler` (scheduled flights + notifications), `LocationManager` (nearest home airport), `WeatherService` — Open-Meteo (free, no API key), then `.clear`; a `XCTestConfigurationFilePath` guard keeps unit tests offline. Keep that fallback chain intact. **Do not add WeatherKit back.** The app never held the entitlement, so the old `import WeatherKit` only ever linked the framework and threw at runtime while Open-Meteo supplied every shipped reading — and that stray link drew an App Review 5.2.5 rejection for missing Apple Weather attribution. Open-Meteo's CC BY 4.0 credit lives in the Settings "Weather data" section, deliberately off the flight screen. `Shaders.metal` holds the window's atmospheric-haze `colorEffect` (returns **premultiplied** alpha — required, or the scene washes out; building `.metal` needs the Metal toolchain: `xcodebuild -downloadComponent MetalToolchain`).
...
**Audio (`Voyage/Audio/`):** the soundscape is synthesized at runtime — `CabinAudioEngine` renders the ambience bed and every one-shot (chime, rip, printer, spool, touchdown, thunk) into buffers it generates itself, `EngineTone` adds the phase-keyed detuned-sine layer under the bed, and `Haptics` drives the taptic cues. The PA is the one recorded-sounding surface but is not a recording either: `Announcer` renders speech offline with `AVSpeechSynthesizer.write`, pushes the buffers through a cabin-speaker filter (band-passed, softly saturated), and hands them to `CabinAudioEngine.playPA`. Which audio assets are bundled depends on the branch — check `Voyage/Resources/` before assuming; the `main` lineage bundles none and the PA is entirely synthesized, while the release lineage adds recorded crew lines with the synthesized path as the fallback for a user-picked device voice. Audio is **foreground-only**: there is deliberately no `audio` entry in `UIBackgroundModes` (App Review 2.5.4 rejected the unearned declaration — strict mode diverts 30s after backgrounding, so nothing needs to keep playing). Do not re-add it. The session is `.playback` + `.mixWithOthers`, and `CabinAudioEngine.resumeIfInterrupted()` restarts the cabin when the app returns to the foreground.
