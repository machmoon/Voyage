...
/// Runtime inputs used to choose a provider. The decision stays independent
/// of GoogleMaps3D so the experimental SDK never leaks into session logic.
struct WorldSceneryProviderPolicy {
    static func provider(
        ...
        hasGoogleMaps3DKey: Bool,
        googleMaps3DAllowed: Bool
        ...
        // Keep streamed scenery alive under moderate pressure, but avoid the
        // experimental photogrammetry renderer once the OS reports serious
        // pressure. MapKit remains a substantially cheaper online fallback.
        let thermallySafeForGoogle = thermalState != .serious && thermalState != .critical
        if hasGoogleMaps3DKey, googleMaps3DAllowed, thermallySafeForGoogle {
            return .googleMaps3D
        }
        return .mapKit
    }
...
enum WorldSceneryConfiguration {
    static let googleMaps3DAPIKeyInfoKey = "GoogleMapsAPIKey"

    /// Resolve through Info.plist in production and the process environment in
    /// previews/tests. Unexpanded build-setting placeholders are rejected so
    /// they never get passed to the Google SDK as credentials.
    static func googleMaps3DAPIKey(
        bundle: Bundle = .main,
        processInfo: ProcessInfo = .processInfo
    ) -> String? {
        let candidates = [
            bundle.object(forInfoDictionaryKey: googleMaps3DAPIKeyInfoKey) as? String,
            processInfo.environment["GOOGLE_MAPS_API_KEY"]
        ]

        return candidates.compactMap { $0 }
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .first(where: isUsableAPIKey)
    }

    static var googleMaps3DAllowedByProcess: Bool {
        let processInfo = ProcessInfo.processInfo
        return !processInfo.arguments.contains("-VoyageDisableGoogleMaps3D")
            && processInfo.environment["VOYAGE_DISABLE_GOOGLE_MAPS_3D"] != "1"
    }
    ...
    static func isUsableAPIKey(_ value: String) -> Bool {
        !value.isEmpty &&
            !value.contains("$(") &&
            !value.localizedCaseInsensitiveContains("YOUR_API_KEY") &&
