    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section {
                    Toggle(isOn: Binding(
                        get: { settings.realWorldTwinEnabled },
                        set: { settings.realWorldTwinEnabled = $0 }
                    )) {
                        Label("Real-World Twin", systemImage: "globe.americas.fill")
                    }
                } header: {
                    Text("Window")
                } footer: {
                    Text("Streams route-aware 3D terrain continuously from takeoff through landing. Voyage switches to Apple imagery or its offline world automatically when Google 3D, credentials, or the network are unavailable.")
                }
