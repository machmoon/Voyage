...
[[ stitchable ]] half4 passengerWing(float2 position, half4 color,
    ...
    float root = 0.91 - uv.x * 0.43 + bank * 0.0022;
    float thickness = mix(0.115, 0.018, smoothstep(0.05, 0.98, uv.x));
    float leading = root - thickness * 0.58;
    float trailing = root + thickness;
    float spanMask = smoothstep(0.035, 0.09, uv.x) * (1.0 - smoothstep(0.93, 0.995, uv.x));
    ...
    float2 engineP = (uv - float2(0.30, root + 0.095)) / float2(0.105, 0.052);
    ...
    float3 paint = mix(float3(0.32, 0.35, 0.39), float3(0.90, 0.92, 0.94), ndl);
    paint += (fresnel * 0.34 + specular * 0.30);
    ...
    float tipGlow = smoothstep(0.042, 0.0, length(uv - float2(0.94, root - 0.005))) * strobe;

    float alpha = saturate(wingMask * 0.98 + engineMask * 0.96 + tipGlow * 0.72);
    float3 rgb = mix(engine, paint, saturate(wingMask / max(alpha, 0.001)));
