...
<p>
  ...
  <img src="QA/qa-02-selected.png" width="170" alt="Route selected">
</p>

The boarding, seat-map, in-flight, and map screenshots are being re-shot — see [QA Screenshots](#qa-screenshots).
...
`QA/*.png` is committed on purpose: it's the current visual state of the app, regenerated by the screenshot tour. Logs (`QA/*.log`, `QA/*.txt`, `QA/uitest/`) are gitignored.

The boarding-pass, seat-map, in-flight, and map screenshots were removed: they were shot before the airline became Voyage Air, so every one of them printed a real carrier's name and flight code on a mock boarding pass. Rerunning `ScreenshotTourUITests` restores the full set.
